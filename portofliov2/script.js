...
setupProjectFilters();
setupMissionFilters();
setupSupplementaryFilters();
setupContact();
setupCopyEmail();
setupCertificationLightbox();

function setupSupplementaryFilters() {
  const buttons = document.querySelectorAll(".supp-filter-btn");
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      items.forEach((item) => {
        const cats = String(item.dataset.cat || "").split(/\s+/).filter(Boolean);
...
function setupMissionFilters() {
  const buttons = document.querySelectorAll(".mission-filter-btn");
